...
const ZKPMasterPasswordPopup: React.FC<ZKPMasterPasswordPopupProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-slate-700">
                {/* Header */}
                ...
                <div className="flex items-center justify-center mb-6">
                    <div className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-medium ${step === 'create' ? 'bg-blue-500 text-white' : 'bg-green-500 text-white'
                        }`}>
                        {step === 'create' ? '1' : <Check className="w-3 h-3" />}
                    </div>
                    <div className={`w-10 h-0.5 mx-2 ${step === 'confirm' ? 'bg-green-500' : 'bg-slate-600'
                        }`} />
                    <div className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-medium ${step === 'confirm' ? 'bg-blue-500 text-white' : 'bg-slate-600 text-slate-400'
                        }`}>
                        2
                    </div>
                </div>
                    ...
                    <div className="flex justify-center space-x-2">
                        {(step === 'create' ? pin : confirmPin).map((digit, index) => (
                            <input
                                ...
                                className="w-10 h-10 text-center text-lg font-bold bg-slate-700 border border-slate-600 rounded-lg text-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all"
                                placeholder="•"
                            />
                        ))}
                    </div>
                ...
                {/* Error Message */}
                {error && (
                    <div className="flex items-center space-x-2 text-red-400 text-xs mb-3 p-2 bg-red-500/10 rounded-lg border border-red-500/20">
                ...
                {/* Security Info */}
                <div className="bg-slate-700/50 rounded-lg p-2 mb-5">
                    <div className="flex items-start space-x-2">
                        <Lock className="w-3 h-3 text-blue-400 mt-0.5 flex-shrink-0" />
                        <div className="text-xs text-slate-300">
                            <p className="font-medium mb-0.5">Zero-Knowledge Security</p>
                            <p className="text-xs">Your PIN never leaves your device. Even we cannot see your passwords without it.</p>
                        </div>
                    </div>
                </div>
                    ...
                    {step === 'create' ? (
                        <>
                            ...
                            <button
                                ...
                                className="flex-1 px-3 py-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 disabled:from-slate-600 disabled:to-slate-600 text-white rounded-lg text-sm font-medium transition-all disabled:cursor-not-allowed"
                            >
                                Next
                            </button>
                        </>
                        ...
                        <>
                            ...
                            <button
                                ...
                                className="flex-1 px-3 py-2 bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 disabled:from-slate-600 disabled:to-slate-600 text-white rounded-lg text-sm font-medium transition-all disabled:cursor-not-allowed flex items-center justify-center"
                            >
                                {isLoading ? (
